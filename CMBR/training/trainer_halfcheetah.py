...
from CMBR.utils.module import get_parameters, FreezeParameters
from CMBR.utils.algorithm import compute_return
...
class Trainer(object):
    ...
    def collect_seed_episodes(self, env):
        ...
        for i in range(self.seed_steps):
                ...
                if done_:
                    self.buffer.add(s, a, reward, cost, terminated)
                    s, _  = env.reset()
                    done_ = False
                else:
                    self.buffer.add(s, a, reward, cost, terminated)
