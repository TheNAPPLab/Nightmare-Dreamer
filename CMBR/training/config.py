...
class BaseSafeConfig():
    ...
    #training desc
    train_steps: int = int(5e6)
    train_every: int = 40  #50   #reduce this to potentially improve sample requirements
    collect_intervals: int = 5 
    batch_size: int = 50
    seq_len: int = 50
    eval_episode: int = 4
    ...
    lambda_: float = 0.95
    horizon: int = 15
    lr: Dict = field(default_factory=lambda:{'model':2e-4, 'actor':4e-5, 'critic':1e-5,'lambda_lr':0.035})
...
class PandaSafeConfig():
    ...
    actor_grad_mix: int = 0.0
    # actor_entropy_scale: float = 1e-3
    actor_entropy_scale: float = 1e-4 # for continous control
