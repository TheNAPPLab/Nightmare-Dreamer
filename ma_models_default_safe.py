...
class ImagBehavior(nn.Module):
  ...
  def __init__(self, config, world_model, stop_grad_actor=True, reward=None, cost = None):
    ...
    #value network for safe actor
    self.value_safep = networks.DenseHead(
        feat_size,  # pytorch version
        [], config.value_layers, config.units, config.act,
        config.value_head)
    
    #cost value network for safe actor
    ...
    if config.slow_value_target or config.slow_actor_target:
        ...
        self._slow_value_safep = networks.DenseHead(
          feat_size,  # pytorch version
          [], config.value_layers, config.units, config.act)
    ...
    self._value_safep_opt = tools.Optimizer(
        'value_safep', self.value_safep.parameters(), config.value_lr, config.opt_eps, config.value_grad_clip,
        **kw)
  ...
  def _train(
      self, start, objective=None, constrain = None, action=None, reward=None, imagine=None, tape=None, repeats=None, mean_ep_cost = None, training_step = 0):
    ...
    with tools.RequiresGrad(self.safe_actor):
      with torch.cuda.amp.autocast(self._use_amp):
        ...
        #compute cost and reward from safe actor rollouts
        ...
        target_safep, weights = self._compute_target(
            ...
            self._config.slow_actor_target)
        #shared weights
        target_cost, _ = self._compute_target_cost(
  ...
  def _compute_target_cost(
    ...
    if slow:
      cost_value = self._slow_cost_value(imag_feat).mode()
    else:
      cost_value = self.cost_value(imag_feat).mode()
    target_cost = tools.lambda_return(
        cost[:-1], cost_value[:-1], discount[:-1],
        bootstrap=cost_value[-1], lambda_=self._config.discount_lambda, axis=0)
    weights = torch.cumprod(
        torch.cat([torch.ones_like(discount[:1]), discount[:-1]], 0), 0).detach()
    return target_cost, weights
  ...
  def _compute_safe_actor_loss(
      ...
      weights, target_cost):
        ...
        policy = self.safe_actor(inp)
        actor_ent = policy.entropy()
        target = torch.stack(target, dim=1)
        target_cost = torch.stack(target_cost, dim=1)
        metrics["mean_target_under_safe_policy"] = to_np(torch.mean(target))
        metrics["mean_target_cost"] = to_np(torch.mean(target_cost))
        metrics["max_target_cost"] = to_np(torch.mean(target_cost))
        actor_target = self._config.alpha1  * target
        actor_target -= self._config.alpha2 * target_cost
  ...
  def _update_slow_target(self):
    if self._config.slow_value_target or self._config.slow_actor_target:
      if self._updates % self._config.slow_target_update == 0:
        ...
        #slow update value fn for safe actor
        mix = self._config.slow_target_fraction
        for s, d in zip(self.value_safep.parameters(), self._slow_value_safep.parameters()):
          d.data = mix * s.data + (1 - mix) * d.data
      self._updates += 1
