...
class TransitionBuffer():
    def __init__(
        ...
        self.cost = np.empty((capacity,), dtype=np.float32) 
    ...
    def _shift_sequences(self, obs, actions, rewards, terminals):
        ...
        return obs, actions, rewards, costs, terminals

#Following objects are not used

Episode = namedtuple('Episode', ['observation', 'action', 'reward', 'cost', 'terminal', 'length'])  
...
class EpisodicBuffer():
    ...
    def sample(self):
        ...
        cos_batch = np.empty([seq_len, batch_size], dtype=np.float32)
        term_batch = np.empty([seq_len, batch_size], dtype=bool)
        for ind, episode in enumerate(episode_list):
            obs_batch[:,ind], act_batch[:,ind], \
                rew_batch[:,ind], cos_batch[:,ind], term_batch[:,ind] = self._sample_seq(episode, seq_len)
        return obs_batch, act_batch, rew_batch ,cos_batch, term_batch
...
class FluidEpisodicBuffer():
    ...
    def sample(self):
        ...
        for i,idx in enumerate(episode_idx):
            obs_batch[:,i], act_batch[:,i], rew_batch[:,i],\
              cos_batch[:,i], term_batch[:,i] = self._sample_seq(self.buffer[idx], self.lengths[idx], seq_len)
        return obs_batch, act_batch, rew_batch ,cos_batch, term_batch
